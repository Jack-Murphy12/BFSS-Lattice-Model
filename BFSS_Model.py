...
iterations = 50000              # number of metropolis iteratoins
burn_in = 0#int(iterations/10)    # how many of the initial iterations will be discarded
Dim = 9                         # number of spacial dimensions
Lambda = 22                     # number of sites on lattice
N = 16                          # size of matrices
beta = 1/0.6                      #
a = beta/Lambda                 #
m = 1.0                         #
eps =0.000003                       # leapfrog integration step size
...
def internal_energy(x, a):
    """Gives the internal energy E/(N^2)"""
    Dim, Lambda, N = len(x), len(x[0]), len(x[0][0])
    A = 0            # will use to get commutator term
    
    for n in range(Lambda):         # summing over lattice sites
        for i in range(Dim):        # summing over dimensions
            for j in range(Dim):
                if i!=j:            # commutator cancels out if i=j
                        A += np.trace(x[i][n] @ x[j][n] @ x[i][n] @ x[j][n]
                                      +x[j][n] @ x[i][n] @ x[j][n] @ x[i][n]
                                      -x[i][n] @ x[j][n] @ x[j][n] @ x[i][n]
                                      -x[j][n] @ x[i][n] @ x[i][n] @ x[j][n])
    return np.real(-3/(4*N*Lambda)*A)


def polyakov(theta):
    N = len(theta)
    return 1/N*(sum(np.exp(1j*theta)))


x = position_array(Dim, Lambda, N, 0.007)      # initial position
theta = np.random.normal(0.0, 0.007, N)   # initial theta
no_accepted=0
S = [S_b(x, theta)]
eigen = [np.linalg.eigvals(x)]
E = [internal_energy(x, a)]
...
for i in range(1, iterations):
    ...
    #E.append(internal_energy(x, a))
    
acc_rate = no_accepted/iterations*100
S, E, iterations  = S[burn_in:], E[burn_in:], iterations-burn_in
